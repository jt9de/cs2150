...
string BinarySearchTree::pathToHere(BinaryNode*& n, const string& x) const{
    // YOUR IMPLEMENTATION GOES HERE
    if (n == NULL){
        return "";
    }else if (x < n -> value){
...
BinaryNode* BinarySearchTree::findhere(BinaryNode*& n, const string& x) const{
    if (n == NULL){
        return NULL;
    }else if (n -> value > x){
...
int BinarySearchTree::size(BinaryNode*& n){
    if (n == NULL){
        return 0;
    }else{
